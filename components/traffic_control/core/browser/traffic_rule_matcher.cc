...
void TrafficRuleMatcher::Rebuild(
  ...
  for (size_t rule_index = 0; rule_index < rules.size(); ++rule_index) {
    ...
    for (std::string_view line :
      ...
      // |rules_| index matches the input index for clones we pushed.
      pattern_id_to_rule_index_[next_id] = rules_.size() - 1;
    }
  }
...
mojom::TrafficRulePtr TrafficRuleMatcher::FindMatchingRule(
  ...
  return rules_[best_index]->Clone();
}
